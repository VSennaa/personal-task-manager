...
export function compareTasksForSort<T extends TaskForSort>(a: T, b: T): number {
...
export function sortTasks<T extends TaskForSort>(tasks: T[]): T[] {
  return tasks.filter((task) => VISIBLE_STATUSES.has(task.status)).slice().sort(compareTasksForSort);
}
